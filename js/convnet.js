...
(function (global) {
  ...
  var Vol = function (sx, sy, depth, constant) {
...
(function (global) {
  ...
  var img_to_vol = function (img, convert_grayscale) {
    ...
    for (var i = 0; i < pixels.length; i++) {
      pv.push(pixels[i] / 255.0 - 0.5); // normalize image pixels to [-0.5, 0.5]
    }
...
(function (global) {
  ...
  var FullyConnLayer = function (opt) {
    ...
    this.out_depth =
      typeof opt.num_neurons !== "undefined" ? opt.num_neurons : opt.filters;
...
(function (global) {
  ...
  // Implements identity nonlinearity elementwise
  // x -> x
  // the output is in (-inf, inf)
  var IdentityLayer = function (opt) {
    var opt = opt || {};
    ...
    this.out_sy = opt.in_sy;
    this.out_depth = opt.in_depth;
    this.layer_type = "identity";
  };
  IdentityLayer.prototype = {
    forward: function (V, is_training) {
      this.in_act = V;
      var V2 = V.clone();
      this.out_act = V2;
  ...
  global.IdentityLayer = IdentityLayer;
})(convnetjs);
      ...
      var desugar = function () {
        ...
        for (var i = 0; i < defs.length; i++) {
          ...
          if (typeof def.activation !== "undefined") {
            ...
            } else if (def.activation === "identity") {
              new_defs.push({ type: "identity" });
      ...
      for (var i = 0; i < defs.length; i++) {
        ...
        switch (def.type) {
          ...
          case "sigmoid":
            ...
            break;
          case "identity":
            this.layers.push(new global.IdentityLayer(def));
            break;
          case "tanh":
...
(function (global) {
  ...
  var Trainer = function (net, options) {
    ...
    this.method =
      typeof options.method !== "undefined" ? options.method : "sgd"; // sgd/adam/adagrad/adadelta/windowgrad/netsterov
  ...
  Trainer.prototype = {
    train: function (x, y) {
      ...
      if (this.regression && y.constructor !== Array)
        console.log(
          "Warning: a regression net requires an array as training output vector."
        );
      ...
      if (this.k % this.batch_size === 0) {
        ...
        for (var i = 0; i < pglist.length; i++) {
          ...
          var l2_decay_mul =
            typeof pg.l2_decay_mul !== "undefined" ? pg.l2_decay_mul : 1.0;
          var l1_decay_mul =
            typeof pg.l1_decay_mul !== "undefined" ? pg.l1_decay_mul : 1.0;
          ...
          for (var j = 0; j < plen; j++) {
            ...
            if (this.method === "adam") {
              ...
              var dx =
                (-this.learning_rate * biasCorr1) /
                (Math.sqrt(biasCorr2) + this.eps);
              p[j] += dx;
            } else if (this.method === "adagrad") {
              // adagrad update
              gsumi[j] = gsumi[j] + gij * gij;
              var dx =
                (-this.learning_rate / Math.sqrt(gsumi[j] + this.eps)) * gij;
              p[j] += dx;
              ...
              var dx =
                (-this.learning_rate / Math.sqrt(gsumi[j] + this.eps)) * gij; // eps added for better conditioning
              p[j] += dx;
            } else if (this.method === "adadelta") {
              gsumi[j] = this.ro * gsumi[j] + (1 - this.ro) * gij * gij;
              var dx =
                -Math.sqrt((xsumi[j] + this.eps) / (gsumi[j] + this.eps)) * gij;
